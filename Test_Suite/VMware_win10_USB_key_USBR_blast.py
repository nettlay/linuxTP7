...
def edit_connection(case):
    ...
    if window is None:
        blast_usbr_log.error('not find edit connection window')
        return 'not find edit connection window'
    pag.press('down')
    usb_r = case.image_match('view_usbr', 3)
    if usb_r is None:
        blast_usbr_log.error('not find usbr settings')
    ...
    blast_usbr_log.info('enable usbr settings')
    return True


def check_usb():
    if len(os.popen('ls /media').readlines()) == 0:
        blast_usbr_log.error('not find usb key')
...
def check_file():
    ...
    if 'transfer_check.txt\n' not in os.popen('ls /media/%s' % usb_name).readlines():
        blast_usbr_log.error('file not exist after logoff')
    ...
    for line in file.readlines():
        if line == 'usbkey redirection test':
            ...
            blast_usbr_log.info('usbr test success,copy file to usb success')
            return True
        else:
            file.close()
            os.remove('/media/%s/transfer_check.txt' % usb_name)
            blast_usbr_log.error('the content of file is incorrect')
