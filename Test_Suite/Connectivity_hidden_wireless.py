...
def run():
    ...
    log = report.logger()
    wire = wireless.Wireless('R1_Linux_TKIP', 'hidden_wireless_password')
    ...
    if boot_times == '1':
        wire.clear_profiles()
        wire.open_wireless()
        hidden_connect = wire.connect_hidden_wireless()
        if hidden_connect is not True:
            report.fail_report('Fail:%s' % hidden_connect, file_name)
            write()
        ...
        if wire.ping() is False:
            report.fail_report('Fail:ping to wireless fail', file_name)
            write()
            wire.switch_wired('remove', 'up')
    ...
    if boot_times == '2':
        time.sleep(5)
        if wire.ping() is False:
            report.fail_report('Fail:ping to wireless fail after reboot', file_name)
            write()
            wire.switch_wired('remove', 'up')
            return False
        wire.open_wireless()
