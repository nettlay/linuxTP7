...
def get_redirected_usb_key():
    try:
        log.info('search redirected USB key')
        usb_key = ''
        ...
        for i in range(60):
            for drive in fso.drives:
                ...
                if drive.DriveType == 1 and drive.IsReady:
                    log.info('found the redirected USB Key')
                    usb_key = drive.Path+'/'
                    return usb_key
        if usb_key == '':
            log.error('Cannot find the redirected USB Key')
            # -------------return result to FTP----------------
        ...
        log.error('get_redirected_usb_key', exc_info=True)


def copyfile_to_usb():
        fso = Dispatch("Scripting.FileSystemObject")
        src = "C:\\scripts\\usbr.txt"
        des = fso.BuildPath(get_redirected_usb_key(), "usbr_check.txt")
        if os.path.exists(src):
            os.remove(src)
        if os.path.exists(des):
            os.remove(des)
        text = "1234567890!@#$%^&*()qwertyuiopQWERTYUIOP"
        os.system("echo {} > C:\\scripts\\usbr.txt".format(text))
        fso.CopyFile(src, des, True)
        log.info("copying usbr.txt to USB key")
        return
...
def run_usbr_test():
    try:
        if get_redirected_usb_key():
            copyfile_to_usb()
            fso = Dispatch("Scripting.FileSystemObject")
            usbr = "C:\\scripts\\usbr.txt"
            usbr_check = fso.BuildPath(get_redirected_usb_key(), "usbr_check.txt")
            if fso.FileExists(usbr) and fso.FileExists(usbr_check):
                compare_result = compare(usbr, usbr_check)
        ...
        log.error('run_usbr', exc_info=True)
    except Exception:
        log.error('run_usbr', exc_info=True)
